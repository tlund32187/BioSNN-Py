"""Tiny step throughput benchmark for synapse backends (dev utility)."""
...
def _make_spike_seq(
    *,
    ...
    return (spikes < rate).to(dtype=torch.float32)
...
def _run_case(
    *,
    ...
    fused_label = "n/a" if fused is None else str(fused)
    print(
        f"device={device:4s} synapse={synapse_name:26s} fused={fused_label:5s} "
        f"steps/sec={throughput:10.1f}"
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Tiny step throughput benchmark")
    ...
    _run_case(
        device="cpu",
        synapse_name="DelayedCurrentSynapse",
        fused=None,
        topology=topo_current,
    ...
    for fused in (True, False):
        ...
        _run_case(
            device="cpu",
            synapse_name="DelayedSparseMatmulSynapse",
            fused=fused,
            topology=topo_sparse,
